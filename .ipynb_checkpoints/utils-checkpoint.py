...
def n_community(c_sizes, p_inter=0.01):
    ...
    communities = list(nx.weakly_connected_component_subgraphs(G))
...
def decode_graph(adj, prefix):
    ...
    # degree stats
    G_deg = nx.degree_histogram(G.to_undirected())
    G_deg_sum = [a * b for a, b in zip(G_deg, range(len(G_deg)))]
    avg_deg = sum(G_deg_sum) / G.number_of_nodes() if G.number_of_nodes() > 0 else 0
    print(f'average degree: {avg_deg:.3f}')

    # path and diameter: use weak connectivity for directed graphs
    if G.is_directed() and nx.is_weakly_connected(G):
        Gu = G.to_undirected()
        print(f'average path length: {nx.average_shortest_path_length(Gu):.3f}')
        print(f'diameter: {nx.diameter(Gu)}')
    elif not G.is_directed() and nx.is_connected(G):
        print(f'average path length: {nx.average_shortest_path_length(G):.3f}')
        print(f'diameter: {nx.diameter(G)}')

    # clustering (works on undirected view)
    G_cluster = sorted(list(nx.clustering(G.to_undirected()).values()))
    print(f'average clustering coefficient: {np.mean(G_cluster):.3f}')

    # cycles: use simple_cycles for directed graphs
    cycle_len = []
    if G.is_directed():
        for cycle in nx.simple_cycles(G):
            cycle_len.append(len(cycle))
    else:
...
#     adj_list = [list(adj.keys()) for _, adj in G.adjacency()]
...
# def pick_connected_component_new(G):
#     # Build adjacency list (modern NetworkX)
#     adj_list = [list(adj.keys()) for _, adj in G.adjacency()]
#     for node_id, adj in enumerate(adj_list):
#         id_min = min(adj) if len(adj) > 0 else node_id
#         if id_min < node_id:
#             node_list = list(range(node_id))  # only include nodes prior than node_id
#             G = G.subgraph(node_list).copy()
#             # Modern replacement for connected_component_subgraphs
#             components = [G.subgraph(c).copy() for c in nx.connected_components(G)]
#             G = max(components, key=len)
#             return G
#     return G
...
def pick_connected_component_new(G):
    """
    Returns the largest connected (or weakly connected) component
    for a graph, directed or undirected.
    """
    # Build adjacency list (modern NetworkX)
    adj_list = [list(adj.keys()) for _, adj in G.adjacency()]
    for node_id, adj in enumerate(adj_list):
        id_min = min(adj) if len(adj) > 0 else node_id
        if id_min < node_id:
            node_list = list(range(node_id))  # only include nodes prior to node_id
            G = G.subgraph(node_list).copy()

            # Handle directed and undirected cases correctly
            if G.is_directed():
                comps = nx.weakly_connected_components(G)
            else:
                comps = nx.connected_components(G)

            components = [G.subgraph(c).copy() for c in comps]
            G = max(components, key=len)
            return G
...
#             # I changed this part to handle both directed and undirected graphs because I see gene regulatory'
#             # datasets are directional
#             if graph_list[i].is_directed():
#                 comps = nx.strongly_connected_components(graph_list[i])
#             else:
#                 comps = nx.connected_components(graph_list[i])

#             graph_list[i] = max((graph_list[i].subgraph(c).copy() for c in comps), key=len)
...
def load_graph_list(fname, is_real=True):
    ...
    for i in range(len(graph_list)):
        ...
        if is_real:
            if graph_list[i].is_directed():
                comps = nx.weakly_connected_components(graph_list[i])
            else:
                comps = nx.connected_components(graph_list[i])
            graph_list[i] = max(
                (graph_list[i].subgraph(c).copy() for c in comps),
                key=len
            )
            graph_list[i] = nx.convert_node_labels_to_integers(graph_list[i])
        else:
...
def mean_graph_discrepancy(real_graphs, generated_graphs):
    """
    Compute mean discrepancy between two sets of graphs
    based on node count, edge count, degree, and clustering stats.
    """
    def get_stats(graphs):
        return {
            ...
            "degree_mean": np.array([np.mean([d for _, d in g.degree()]) for g in graphs]),
            "clustering_mean": np.array([
                np.mean(list(nx.clustering(g).values())) if len(g) > 0 else 0 for g in graphs
            ]),
        }

    stats_real = get_stats(real_graphs)
    stats_gen = get_stats(generated_graphs)

    results = {}
    for key in stats_real.keys():
        results[key] = abs(np.mean(stats_real[key]) - np.mean(stats_gen[key]))

    results["overall_mean_discrepancy"] = np.mean(list(results.values()))
    return results
