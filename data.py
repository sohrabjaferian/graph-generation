...
def encode_adj_flexible(adj, max_prev_node=None, make_undirected=True):
    """
    Encode adjacency for GraphRNN-style sequence.
    - adj: square numpy array (0/1), shape [N, N]
    - max_prev_node: optional window size; if None, use full (i) length each step
    - make_undirected: if True, symmetrize A := (A + A^T > 0), diag=0

    Returns:
      enc: float32 array of shape [N-1, W] where W = max_prev_node or varying length
    """
    A = np.asarray(adj, dtype=np.int64)
    N = A.shape[0]
    assert A.shape[0] == A.shape[1], "adj must be square"

    if make_undirected:
        A = ((A + A.T) > 0).astype(np.int64)
    np.fill_diagonal(A, 0)

    # We encode row i looking only to previous nodes [0..i-1]
    rows = []
    for i in range(1, N):
        prev = A[i, :i]  # shape (i,)
        if max_prev_node is None:
            # no fixed window → just use full prefix
            rows.append(prev.astype(np.float32))
            continue

        # fixed window on the right
        W = max_prev_node
        take = min(W, i)
        adj_slice = prev[-take:]  # shape (take,)

        # where are the ones?
        non_zero = np.flatnonzero(adj_slice)

        if non_zero.size == 0:
            # no previous neighbors → put zeros in the window
            padded = np.zeros((W,), dtype=np.float32)
            padded[-take:] = 0.0
            rows.append(padded)
            continue

        # right-aligned placement consistent with original code intent
        padded = np.zeros((W,), dtype=np.float32)
        padded[-take:] = adj_slice.astype(np.float32)
        rows.append(padded)

    # If variable-length (no max_prev_node), pad to a rectangle for batching
    if max_prev_node is None:
        maxw = max(r.shape[0] for r in rows) if rows else 0
        out = np.zeros((len(rows), maxw), dtype=np.float32)
