...
def load_ground_truth(dir_input, dataset_name, model_name='GraphRNN_RNN'):
    ...
    if model_name=='Internal' or model_name=='Noise' or model_name=='B-A' or model_name=='E-R':
        ...
        fname_test = "/train_data/" + model_name + '_' + dataset_name + '_' + str(args.num_layers) + '_' + str(
                hidden) + '_test_' + str(0) + '.dat'
    try:
        graph_test = utils.load_graph_list(fname_test, is_real=True)
    except Exception as e:
        print(f'Failed to load graph list from {fname_test}')
        print('Error:', e)
        logging.warning(f'Failed to load: {fname_test}, Error: {e}')
        return None
...
def evaluation_epoch(dir_input, fname_output, model_name, dataset_name, args, is_clean=True, epoch_start=1000,epoch_end=3001,epoch_step=100):
    with open(fname_output, 'w+') as f:
        ...
        if model_name=='Internal' or model_name=='Noise' or model_name=='B-A' or model_name=='E-R':
            ...
            fname_test = dir_input + model_name + '_' + dataset_name + '_' + str(64) + '_test_' + str(0) + '.dat'
        # else:
        #     fname_test = './real_data/train_data.dat'   # <--- Directly load your actual training graphs
        

        else:
            fname_test = './real_data/train_data.dat'
            try:
                graph_test = utils.load_graph_list(fname_test, is_real=True)
            except Exception as e:
                print(f"[ERROR] Failed to load test graph from: {fname_test}")
                print(f"[DETAIL] Exception: {type(e).__name__}: {e}")
                logging.warning(f"[ERROR] Failed to load: {fname_test}, Exception: {e}")
                return None
...
if __name__ == '__main__':
    ...
    train_dir = "./real_data/"           # where real / training graphs are stored
    synthetic_dir = "./synthetic_data/"   # where predicted / generated graphs are stored
    ...
    if not os.path.isdir('logs/'):
        os.makedirs('logs/')
    logging.basicConfig(filename='logs/evaluate_' + time_now + '.log', level=logging.INFO)

    # ====== Ground Truth Export Mode ======
    if prog_args.export:
        ...
        if prog_args.graph_type == 'grid':
            ...
            graphs = [caveman_special(i, j, p_edge=0.3)
                      for i in range(2, 3) for j in range(30, 81) for _ in range(10)]
            utils.export_graphs_to_txt(graphs, output_prefix)
            ...
            input_path = train_dir + real_graph_filename
            g_list = utils.load_graph_list(input_path)
            utils.export_graphs_to_txt(g_list, output_prefix)

    # ====== Kronecker Graph Conversion ======
    elif prog_args.kron_dir != '':
    ...
    # ====== Evaluate a Single Graph File ======
    elif prog_args.test_file != '':
        graphs = utils.load_graph_list(prog_args.test_file)
        eval_single_list(graphs, dir_input=train_dir, dataset_name='grid')

    # ====== Main Evaluation Mode ======
    else:
        # Ensure output folder exists in synthetic data directory
        os.makedirs(synthetic_dir + 'eval_results', exist_ok=True)

        print("\nStarting evaluation...")
        print(f" → Real graphs from: {train_dir}")
        print(f" → Synthetic graphs from: {synthetic_dir}")
        print(f" → Results saved to: {synthetic_dir}eval_results/\n")

        evaluation(args_evaluate,
                   dir_input=synthetic_dir,
                   dir_output=synthetic_dir + "eval_results/",
